...
\begin{questionbox}
Given a dataset with columns \tech{date}, \tech{product}, \tech{revenue}, and \tech{units\_sold}, how would you structure a query to find the top 3 trending products by revenue growth?
\end{questionbox}

\begin{solutionbox}
We can construct our solution as follows:
\begin{enumerate}
  \item \tech{Initialize the data structure} --- Load the dataset and parse date fields
  \item \tech{Validate input parameters} --- Ensure all required columns exist
  \item \tech{Process the request} --- Calculate revenue growth per product over time
  \item \tech{Return formatted output} --- Sort by growth rate and return top 3
\end{enumerate}
\end{solutionbox}
...
Dataset: sales\_2025.csv\\
Columns: date, product, revenue, units\_sold
\end{promptbox}
